...
make_rec.make_list(os.path.join(config.data_dir, "comic"), config.data_dir, True, [".jpg", ".png"], 1, 0.8)
image_list = make_rec.read_list(os.path.join(config.data_dir, "comic_train.lst"))
make_rec.write_list(image_list, os.path.join(config.data_dir, config.train_dataset))
image_list = make_rec.read_list(os.path.join(config.data_dir, "comic_val.lst"))
make_rec.write_list(image_list, os.path.join(config.data_dir, config.val_dataset))
